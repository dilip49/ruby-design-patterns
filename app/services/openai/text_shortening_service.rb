...
class TextShorteningService
  ...
  def shorten_text(text, characters_limit)
    ap "Calling ChatGPT API with text: #{text}"
    ...
    if response.success? && content.length > characters_limit
      ap "Result exceeds #{characters_limit} characters, retrying..."
      retry_shortening(conversation_history, characters_limit, content)
    else
      content
    end
  ...
  def build_initial_user_message(text, characters_limit)
    [
      {
        role: "user",
        content: "Please shorten the below German medical title to a maximum of #{characters_limit} characters, including spaces and special characters. Verify that the response meets the character limit before replying. Also make sure to only reply with the abbreviation. #{text}"
      }
    ]
  end
  ...
  def prepare_new_prompt(content, characters_limit)
    {
      role: "user",
      content: "This is more than #{characters_limit} characters. Please make another iteration and make sure to stay below the character limit. Also make sure to only reply with the abbreviation as before."
    }
  end
end
